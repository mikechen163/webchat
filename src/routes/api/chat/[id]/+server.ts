...
export async function POST({ request, params, fetch, locals }) {
  ...
  // const { content, modelId, temperature = 0.7, max_tokens,   isSearchAnalysis = false } = await request.json();
   ...
   // console.log('Effort value set to:', effort);
    ...
    if (enableIntelligentSearch && isSearchAnalysis) {
      console.log('[Intelligent Search] Starting simplified intelligent search analysis...');
      
      try {
        // Simple rule-based decision logic for now
        ...
        const needsSearch = searchKeywords.some(keyword => contentLower.includes(keyword.toLowerCase())) &&
                           !noSearchKeywords.some(keyword => contentLower.includes(keyword.toLowerCase()));
        
        console.log('[Intelligent Search] Decision:', { 
          needsSearch, 
          content: contentLower,
          method: 'rule-based (simplified)'
        });
        
        const decision = {
          requiresSearch: needsSearch,
          reasoning: needsSearch ? 
            'Query requires current or factual information that benefits from web search' : 
            'Query can be answered with general knowledge or is creative/educational in nature',
          confidence: 85
        };
        
        console.log('[Intelligent Search] Decision result:', decision);
        
        // Return the decision as the response
        return json(decision);
        
      } catch (error) {
        console.log('[Intelligent Search] Simplified decision error:', error);
      }
    }
  ...
  // 获取历史消息并保存用户消息
    ...
    // Handle intelligent search results from frontend
    let enhancedContent = content;
    let hasSearchResults = false;
    
    if (enableIntelligentSearch && !isSearchAnalysis) {
      console.log('[Server] Processing intelligent search request');
      
      // Check if this is a search-enhanced request
      if (content.includes('基于前面提供的搜索结果') || content.includes('Based on the search results provided')) {
        console.log('[Server] Detected search-enhanced content request');
        hasSearchResults = true;
        
        // For now, we'll let the frontend handle the search results formatting
        // The frontend should include the search results in the content
...
{"tool":"execute_python","code":"print('Hello world!')","timeout":5}`;
    ...
    const messages = [{ role: 'system', content: toolDescription }, ...history, { role: 'user', content: enhancedContent }];
