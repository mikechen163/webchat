<script lang="ts">
	...
	import { ArrowUp, Trash2, Search, X, Info, Plug } from 'lucide-svelte';
	...
	async function toggleMcpServer(server: any) {
		try {
			// Optimistic update
			const newValue = !server.userEnabled;
			availableMcpServers = availableMcpServers.map((s) =>
				s.id === server.id ? { ...s, userEnabled: newValue } : s
			);

			const res = await fetch('/api/user/mcp-servers', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ mcpServerId: server.id, enabled: newValue })
			});
			...
			showToast({
				title: 'Error',
				description: 'Failed to update MCP server status',
				type: 'error'
			});
		}
	}
...
Important: Keep the response concise and ensure it's valid JSON.`;
		...
		const response = await fetch(`/api/chat/${$page.params.id}`, {
			...
			body: JSON.stringify({
				...
				max_tokens: 2000,
				system:
					'You are a search results analyzer. Return only valid JSON, no explanation or formatting.',
...
`;
			...
			if (
				...
				analysis.subtasks &&
				Array.isArray(analysis.subtasks) &&
					...
					if (subtask && typeof subtask.keywords === 'string' && subtask.keywords.trim()) {
						...
						if (analysis.considerFreshness) {
							...
							const hasTimeIndicator =
								/202[3-4]|recent|latest|current|today|yesterday|week|month/i.test(searchQuery);
							if (!hasTimeIndicator) {
						...
						console.log(
							`[Chat] Searching subtask ${i + 1}/${subtasksToSearch.length}: ${searchQuery}`
						);
						try {
	...
	async function checkDirectory() {
		...
		checkingDir = true;
		try {
			const res = await fetch('/api/mcp/listdir', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
...
${formattedResults}
					...
					messages = messages.map((msg) =>
						msg.id === tempAssistantMsgId
							? {
									...msg,
									content:
										'I apologize, but I was unable to perform the web search. Please try again later.'
								}
							: msg
					);
					sending = false;
			...
			if (abortController) {
				// Only reset if not already handled by handleStop
				sending = false;
...
<div class="flex h-[var(--viewport-height)] w-full flex-col">
	<div class="flex flex-1 flex-col overflow-hidden">
		<!-- Header -->
		<div class="flex-shrink-0 border-b">
			<div class="flex w-full items-center justify-center px-2 py-3 md:px-4 md:py-4">
				{#if editingTitle}
					<form on:submit|preventDefault={updateTitle} class="flex items-center gap-2">
						<Input
							bind:value={newTitle}
							class="w-64"
							autofocus
				...
				{:else}
					<button
						type="button"
						class="max-w-[80%] truncate text-center text-lg font-semibold hover:text-gray-600 md:text-xl"
		...
		<div class="flex flex-1 overflow-hidden">
			...
			>
				<div class="mx-auto w-full space-y-4 px-3 py-3 md:max-w-3xl md:px-4 md:py-4 lg:max-w-4xl">
			...
			{#if webSearchMode && showSearchProgress && searchProgress.status !== 'idle'}
				<div class="hidden w-80 overflow-y-auto border-l bg-gray-50 md:block lg:w-96">
				...
				<div
					class="fixed inset-0 z-50 bg-black/30 md:hidden {showSearchProgress ? 'block' : 'hidden'}"
				>
					<div
						class="absolute bottom-0 right-0 top-0 w-[85%] max-w-md overflow-y-auto bg-white shadow-lg"
					>
						<SearchProgressDisplay
							{searchProgress}
							onClose={() => (showSearchProgress = false)}
		...
		{#if sending}
			<div class="shrink-0">
				<div class="w-full px-3 py-2 text-sm text-gray-500 md:px-4">Bot is typing...</div>
			</div>
		{/if}

		<!-- Input part -->
		<div class="sticky bottom-0 left-0 right-0 z-10 flex-shrink-0 border-t bg-white">
			<div class="mx-auto w-full px-3 py-3 md:max-w-3xl md:px-4 md:py-4 lg:max-w-4xl">
				<!-- Tool bar -->
				<div class="mb-2 flex items-center gap-2 overflow-x-auto pb-1 text-sm text-gray-600">
					{#each tools as tool}
						<button
							class="rounded-full px-2 py-1 md:px-3 md:py-1.5
                {tool.id === 'websearch' && webSearchMode
								? 'bg-blue-100 text-blue-600'
								: 'hover:bg-gray-100'} 
                flex items-center gap-1 whitespace-nowrap md:gap-1.5"
							on:click={() => {
					...
					<div class="ml-2">
						...
						<div class="relative inline-block">
							<select
								...
								class="rounded-md border border-gray-200 bg-white px-2 py-1 text-sm md:text-sm"
				...
				<!-- Message input form -->
				<form on:submit|preventDefault={handleSubmit} class="flex max-w-full items-center gap-2">
					<textarea
						bind:value={messageInput}
						placeholder="Send a Message"
						disabled={sending}
						rows="3"
						class="flex-1 resize-none rounded-[24px] border border-gray-200 bg-white px-4 py-3 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 md:px-6 md:text-base"
					...
					<div class="flex gap-2">
						{#if sending}
							<Button
								type="button"
								class="flex h-10 w-10 items-center justify-center rounded-full bg-red-500 p-0 hover:bg-red-600 md:h-12 md:w-12"
								variant="destructive"
								on:click={handleStop}
							>
						...
						<Button
							type="submit"
							disabled={sending}
							class="flex h-10 w-10 items-center justify-center rounded-full bg-blue-500 p-0 hover:bg-blue-600 disabled:cursor-not-allowed disabled:bg-gray-300 md:h-12 md:w-12"
							variant="default"
						>
							{#if sending}
								<div
				...
				<div class="h-2 flex-shrink-0 md:hidden"></div>
			</div>
		</div>
	</div>
</div>
