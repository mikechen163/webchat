<script lang="ts">
	...
	async function addMcpServer() {
		try {
			...
			showToast({ title: 'Error', description: (err as Error).message, type: 'error' });
		}
	}
	...
	async function deleteMcpServer(id: string) {
		try {
			...
			if (response.ok) {
				...
				showToast({ title: 'Success', description: 'MCP Server deleted successfully' });
			} else {
	...
	async function discoverMcpServerTools(id: string) {
		try {
			const response = await fetch(`/api/mcp-servers/${id}/tools`);
			const result = await response.json();
			if (result.success) {
				showToast({ title: 'Success', description: result.message });
				loadMcpServers();
			} else {
	...
	function editMcpServer(server: (typeof mcpServers)[0]) {
		editingMcpServer = true;
	...
	async function discoverModels() {
		...
		try {
			...
			if (result.success) {
				...
				discoveredModelsForProvider = (result.models || []).filter(
					(discoveredModel) => !existingModels.includes(discoveredModel.id)
				);
			...
			showToast({
				title: 'Error',
				description:
					err instanceof Error ? err.message : 'An error occurred during model discovery',
				type: 'error'
			});
		} finally {
			isTestingKey = false;
	...
	async function addModel(modelData) {
		if (!currentProviderForDiscovery) {
			showToast({
				title: 'Error',
				description:
					'Could not add model: provider details are missing. Please try discovering models again.',
				type: 'error'
			});
			return;
		}
		...
		try {
			...
			if (response.ok) {
				...
				discoveredModelsForProvider = discoveredModelsForProvider.filter(
					(m) => m.id !== modelData.model
				);
	...
	async function testProviderKey() {
		try {
			...
			const sanitizedApiKey =
				typeof newProvider.apiKey === 'string'
			...
			if (result.success) {
				...
				const existingModels =
					isEditing && editingProviderId ? providersModels[editingProviderId] || [] : [];
				...
				discoveredModels = (result.models || []).map((m) => {
					...
					const modelExists = existingModels.some((existingModel) => existingModel.model === m.id);
	...
	async function toggleModel(
		...
		enabled: boolean,
		customModel?: { name: string; enabled: boolean }
	) {
		if (customModel) {
			// Handle custom model
			try {
	...
	async function updateProvider() {
		try {
			...
			if (response.ok) {
				const result = await response.json();
				providers = providers.map((p) => (p.id === editingProviderId ? result : p));
				resetProviderForm();
				showToast({
					title: 'Success',
...
<div class="mx-auto h-full max-w-5xl overflow-y-auto px-4 py-8">
	<div class="mb-8">
		<h1 class="mb-2 text-2xl font-bold">Admin Settings</h1>
	...
	<div class="mb-6 flex border-b">
		...
		<button
			class="-mb-px border-b-2 px-4 py-2 text-sm font-medium focus:outline-none"
			class:border-blue-500={activeTab === 'mcp-servers'}
			class:text-blue-600={activeTab === 'mcp-servers'}
			class:border-transparent={activeTab !== 'mcp-servers'}
			class:hover:text-gray-700={activeTab !== 'mcp-servers'}
			on:click={() => {
		...
		<Card>
			...
			<CardContent>
				<div class="space-y-4">
								...
								<div class="flex items-center justify-between rounded-md border p-4">
									...
									<div class="flex gap-2">
										<Button
											variant="outline"
											size="sm"
											on:click={() => handleEditProvider(provider)}
										>
											Modify
										</Button>
										<Button
					...
					{:else}
						<p class="py-4 text-center text-gray-500">No providers configured yet</p>
					{/if}
		...
		<div class="grid grid-cols-1 gap-6 md:grid-cols-4">
			...
			<div class="md:col-span-3">
						...
						<div class="space-y-1">
							...
							<CardDescription>
								{#if selectedProviderId}
									Models for {providers.find((p) => p.id === selectedProviderId)?.name ||
										'selected provider'}
								{:else}
					...
					<CardContent>
						<div class="space-y-4">
							...
							{:else if !selectedProviderId}
								<p class="py-4 text-center text-gray-500">Select a provider to see its models.</p>
							{:else}
				...
				{#if showCustomModelForm}
					<div class="mt-6">
						<h3 class="mb-2 text-lg font-medium">Add Custom Model</h3>
						<Card>
							<CardContent class="pt-6">
								<p class="mb-4 text-sm text-gray-600">
									Could not fetch models automatically. You can add a custom model instead.
								</p>
								<form
	...
	{:else if activeTab === 'mcp-servers'}
		<Card>
			<CardHeader>
				...
				<CardDescription
					>Configure third-party Model Context Protocol (MCP) servers for tool execution</CardDescription
				>
			</CardHeader>
			<CardContent>
				<div class="space-y-4">
					{#if mcpServers.length > 0}
						<div class="grid max-h-[500px] gap-4 overflow-y-auto pr-2">
									...
									<div class="flex items-center justify-between">
										<div class="flex-1">
											<div class="flex items-center gap-2">
												...
												{/if}
												<span
													class="rounded px-2 py-0.5 text-xs {server.enabled
											...
											<p class="text-sm text-gray-500">{server.baseUrl}</p>
											{#if server.description}
												<p class="mt-1 text-sm text-gray-400">{server.description}</p>
											{/if}
										...
										<div class="flex gap-2">
											<Button
												size="sm"
												variant="outline"
												on:click={() => discoverMcpServerTools(server.id)}
											>
												Discover Tools
											</Button>
											<Button
												size="sm"
												variant="outline"
												on:click={() => toggleMcpServer(server.id, !server.enabled)}
											>
												{server.enabled ? 'Disable' : 'Enable'}
											</Button>
											<Button size="sm" variant="outline" on:click={() => editMcpServer(server)}>
												Edit
											</Button>
											{#if !server.isBuiltIn}
												<Button
					...
					{:else}
						<p class="py-8 text-center text-gray-500">No MCP servers configured yet</p>
					{/if}
				</div>
			</CardContent>
			<CardFooter>
		...
		<!-- MCP Server Dialog -->
		{#if showMcpDialog}
			<DialogPrimitive.Root bind:open={showMcpDialog}>
				<DialogPrimitive.Portal>
					<DialogPrimitive.Overlay class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm" />
					<div class="fixed inset-0 z-50 flex items-center justify-center">
						<DialogPrimitive.Content
							class="fixed z-50 w-full max-w-md border bg-background p-6 shadow-lg sm:rounded-lg"
						>
							<div class="mb-4 flex flex-col space-y-1.5">
								<h2 class="text-lg font-semibold">
									{editingMcpServer ? 'Edit' : 'Add'} MCP Server
								</h2>
								<p class="text-sm text-muted-foreground">
									Configure a third-party MCP server for tool execution
								</p>
							</div>

							<form
								on:submit|preventDefault={() =>
									editingMcpServer ? updateMcpServer() : addMcpServer()}
								class="space-y-4"
							>
								<div>
									<label for="mcp-name" class="mb-1 block font-medium">Name</label>
									<Input
										id="mcp-name"
										bind:value={newMcpServer.name}
										placeholder="e.g., Code Execution Server"
										required
									/>
								</div>

								<div>
									<label for="mcp-desc" class="mb-1 block font-medium">Description</label>
									<Input
										id="mcp-desc"
										bind:value={newMcpServer.description}
	...
	{#if showDiscoveredModelsDialog}
		<DialogPrimitive.Root bind:open={showDiscoveredModelsDialog}>
			<DialogPrimitive.Portal>
				...
				<div class="fixed inset-0 z-50 flex items-center justify-center">
					<DialogPrimitive.Content
						class="fixed z-50 flex grid h-[80vh] w-full max-w-4xl flex-col gap-4 border bg-background bg-background p-6 shadow-lg sm:rounded-lg"
					>
						...
						<div class="flex flex-grow flex-row gap-6 overflow-hidden">
							{#if groupedDiscoveredModels}
								<!-- Left Sidebar -->
								<div class="w-1/4 overflow-y-auto border-r pr-4">
									<h4 class="sticky top-0 mb-2 bg-background text-lg font-semibold">Providers</h4>
									{#each Object.keys(groupedDiscoveredModels) as providerName}
										<button
											class="mb-1 w-full rounded-md p-2 text-left text-sm"
							...
							{:else}
								<div class="mt-4 grid max-h-[60vh] w-full gap-2 overflow-y-auto">
						...
						<div class="mt-auto flex justify-end gap-2 border-t pt-4">
							<Button
								...
								on:click={() => {
									showDiscoveredModelsDialog = false;
									selectedDiscoveredProvider = null;
								}}
							>
								Close
							</Button>
						...
						<button
							class="focus:ring-ring absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground"
	...
	{#if showProviderDialog}
		<DialogPrimitive.Root bind:open={showProviderDialog}>
			<DialogPrimitive.Portal>
				...
				<div class="fixed inset-0 z-50 flex items-center justify-center">
					<DialogPrimitive.Content
						class="fixed z-50 grid w-full max-w-lg gap-4 border bg-background bg-background p-6 shadow-lg sm:rounded-lg"
					>
						<div class="flex flex-col space-y-1.5">
						...
						<form class="space-y-4 pt-4" on:submit|preventDefault>
							<div>
								<label for="provider-name" class="mb-1 block font-medium">Provider Name</label>
								<Input
									id="provider-name"
							...
							<div>
								<label for="provider-type" class="mb-1 block font-medium">Provider Type</label>
								<Select
									onSelectedChange={handleProviderTypeChange}
							...
							<div>
								<label for="provider-api-key" class="mb-1 block font-medium">
									API Key
									<span class="text-xs font-normal text-gray-500"
										>{isEditing ? '(leave unchanged to keep current key)' : '(required)'}</span
									>
								</label>
								<div class="flex">
									<Input
										...
										placeholder={isEditing && !newProvider.apiKey
											? '••••••••••••••••'
											: 'Enter API key'}
										required={!isEditing}
										class="flex-grow"
							...
							<div>
								<label for="base-url" class="mb-1 block font-medium">
									Base URL
							...
							{#if newProvider.apiKey}
								<div class="space-y-4 rounded-md border p-4">
									...
									{#if discoveredModels.length > 0}
										<div class="max-h-[240px] space-y-2 overflow-y-auto pr-2">
									...
									{:else if isCustomProvider}
										<div class="max-h-[240px] space-y-2 overflow-y-auto pr-2">
											{#each customModels as model, i}
												<div class="flex items-center gap-2">
													<Input
														...
														class="flex-1"
													/>
													<Button variant="ghost" size="sm" on:click={() => removeCustomModel(i)}>
														<svg
															xmlns="http://www.w3.org/2000/svg"
															width="16"
															height="16"
													...
													<label class="flex items-center space-x-2">
														<input
															...
															on:change={(e) =>
																toggleModel(undefined, e.currentTarget.checked, model)}
															disabled={!model.name}
															class="rounded border-gray-300"
														/>
														<span class="text-sm">Enable</span>
											...
											{/each}
											<Button variant="outline" size="sm" class="w-full" on:click={addCustomModel}>
												Add Custom Model
											</Button>
										</div>
									{/if}
