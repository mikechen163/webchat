<script lang="ts">
	...
	function parseExecutePython(raw: string) {
		try {
			...
			// Extract the code string by finding the "code": "...." value
			const codeKey = '"code":';
			const codePos = raw.indexOf(codeKey, startIdx);
			let code = '';
			let endQuoteIndex = -1;

			const extractStringValue = (key: string, startFrom: number) => {
				const keyPos = raw.indexOf(key, startFrom);
				if (keyPos === -1) return { value: '', endIndex: -1 };

				let q = raw.indexOf('"', keyPos + key.length);
				if (q === -1) return { value: '', endIndex: -1 };
				...
				return { value, endIndex: foundEnd };
			};

			// Extract Code if present
			const codeResult = extractStringValue('"code":', startIdx);
			if (codeResult.endIndex !== -1) {
				code = codeResult.value;
				endQuoteIndex = codeResult.endIndex;
			}

			// Extract Thought if present (for sequentialthinking)
			let thought = '';
			const thoughtResult = extractStringValue('"thought":', startIdx);
			if (thoughtResult.endIndex !== -1) {
				thought = thoughtResult.value;
				// output usually follows the last argument, so update endQuoteIndex if this is later
				if (thoughtResult.endIndex > endQuoteIndex) {
					endQuoteIndex = thoughtResult.endIndex;
				}
			}
	...
	function copyCode() {
		if (toolParsed?.code) copyTextToClipboard(toolParsed.code, (v) => (copiedCode = v));
	}
	function copyResult() {
		if (toolParsed?.result) copyTextToClipboard(toolParsed.result, (v) => (copiedResult = v));
	}
	let copiedThought = false;
	function copyThought() {
		if (toolParsed?.thought) copyTextToClipboard(toolParsed.thought, (v) => (copiedThought = v));
	}
	...
	$: hasReasoning =
		role === 'assistant' && content.includes('<think>') && content.includes('</think>');
	...
	$: htmlContent = ((katexIsReady) => {
		...
		if (!katexIsReady) {
			...
			if (hasReasoning) {
				if (showReasoning) {
					processed = content.replace(/<think>([\s\S]*?)<\/think>/g, (match, reasoningContent) => {
...
<div class="flex gap-4 {role === 'assistant' ? 'bg-gray-50' : ''} group rounded p-4">
	...
	<div class="min-w-0 flex-1">
		<div class="flex items-start justify-between gap-2 md:gap-4">
			<div class="prose w-full max-w-none break-words md:w-auto">
				{#if hasReasoning}
					<!-- Show reasoning toggle if available -->
					<div class="reasoning-toggle mb-1">
						<button
							class="flex items-center gap-1 rounded bg-purple-100 px-2 py-1 text-xs text-purple-700 hover:bg-purple-200"
		...
		<div class="mt-2 flex items-center justify-between text-xs text-gray-500">
			...
			<button
				class="rounded-md p-1 text-gray-500 opacity-0 transition-opacity hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-400 group-hover:opacity-100"
